    ...
    <div className="mx-auto flex max-w-[1440px] flex-col gap-8 px-4 pb-16 pt-24 md:px-6 md:pt-28 lg:px-8">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-cine-text-secondary sm:text-base">
            Escolha um canal para assistir ao vivo.
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="relative max-w-xl">
        ...
        <input
          ...
          className="w-full rounded-xl border border-cine-border bg-cine-surface py-3.5 pl-12 pr-12 text-base text-white outline-none transition-colors placeholder:text-cine-text-muted focus:border-cine-brand focus:ring-1 focus:ring-cine-brand"
          ...
          {highlights.length > 0 && !query && category === "Todos" && (
            <section className="flex flex-col gap-4">
              <div>
                ...
                <p className="text-sm text-cine-text-muted">Confira o que está passando.</p>
              </div>
          ...
          <section className="flex flex-col gap-4">
            <div className="flex items-end justify-between gap-4">
              <div>
                ...
                <p className="text-sm text-cine-text-muted">{filteredChannels.length} {filteredChannels.length === 1 ? "canal disponível" : "canais disponíveis"}</p>
              </div>
            </div>
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
