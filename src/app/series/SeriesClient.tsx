...
import { SERIES_GENRES, type CatalogSort } from "@/config/catalog-filters";
...
async function fetchCatalogPage(
  ...
  const params = new URLSearchParams({ type: "series", genre, sort, page: String(page) });
  const response = await fetch(`/api/catalog?${params}`);
  if (!response.ok) throw new Error("Não foi possível carregar o catálogo.");
  return response.json() as Promise<MediaCatalogPage>;
}

export function SeriesClient({ initialCatalog }: SeriesClientProps) {
  const [selectedGenre, setSelectedGenre] = useState("all");
  const [sortBy, setSortBy] = useState<CatalogSort>("popularity");

  const catalogQuery = useInfiniteQuery({
    queryKey: ["catalog", "series", selectedGenre, sortBy],
  ...
  const allSeries = useMemo(() => {
    ...
    return (catalogQuery.data?.pages.flatMap((page) => page.items) ?? []).filter((item) => {
  ...
  return (
    <div className="flex flex-col gap-10 pb-12">
      ...
      <div className="max-w-[1440px] mx-auto px-4 md:px-6 lg:px-8 w-full flex flex-col gap-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-cine-border pb-4">
          <div>
            ...
            <p className="text-xs sm:text-sm text-cine-text-secondary mt-0.5">
              {totalResults} produções encontradas
            </p>
          </div>
        ...
        {allSeries.length > 0 ? (
          <>
            <MediaGrid items={allSeries} />
            {catalogQuery.hasNextPage && (
              <div className="flex justify-center pt-6">
                <button
                  onClick={() => catalogQuery.fetchNextPage()}
                  disabled={catalogQuery.isFetchingNextPage}
                  className="inline-flex items-center gap-2 px-6 py-2.5 bg-cine-surface hover:bg-cine-surface-elevated text-white font-medium text-sm rounded-xl border border-cine-border transition-colors disabled:cursor-wait disabled:opacity-60"
                >
                  {catalogQuery.isFetchingNextPage && <Loader2 className="h-4 w-4 animate-spin" />}
                  {catalogQuery.isFetchingNextPage ? "Carregando..." : "Carregar mais séries"}
                </button>
              </div>
            )}
          </>
          ...
          <EmptyState
            message={catalogQuery.isLoading ? "Carregando séries..." : "Nenhuma série encontrada nesta categoria."}
          />
        )}
      </div>
    </div>
